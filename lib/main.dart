    ...
    try {
      for (int i = 0; i < contentLength; i += _chunkSize) {
        ...
        if (chunkResponse.statusCode == 206) {
          ...
          // Log each byte in the chunk
          for (var byte in chunkResponse.bodyBytes) {
            _streamedBytes.add(byte);
          }

          // Update UI with streamed byte values
          setState(() {});
        } else {
        ...
        children: <Widget>[
          ...
          Expanded(
            child: ListView.builder(
              itemCount: _streamedBytes.length,
              itemBuilder: (context, index) {
                return ListTile(
                  title: Text('Byte ${index + 1}: ${_streamedBytes[index]}'),
                );
              },
            ),
          ),
